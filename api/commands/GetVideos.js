...
async function execute({msg, client, actionMessage}) {
    ...
    return await TwitchAPIService.getVideos({userId})
        .then(async (response) => {
            const data = response.body.data;
            ...
            if (!datum)
                return msg.reply(`${user.display_name || userId}님의 다시보기가 없습니다.`);
            ...
            edges.forEach((edge, index) => {
                const {node} = edge;
                const {durationMilliseconds: duration, type, description: chapterName, details, video} = node;
