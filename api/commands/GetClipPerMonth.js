...
async function execute({ msg, client, actionMessage }) {
  ...
  const startedAt = moment().subtract(1, 'month');
  ...
  if (!datum) return msg.reply(`${user.display_name || userId}님의 클립이 없습니다. 총 ${data.length.toLocaleString()}개의 클립`);
  ...
  description += '\n\n';
  description += `**__검색기간__** \`${startedAt.format('YYYY-MM-DD HH:mm:ss')}\` ~ \`${endedAt.format('YYYY-MM-DD HH:mm:ss')}\``
