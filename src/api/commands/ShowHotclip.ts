...
async function execute({ msg, client, actionMessage }) {
  ...
  if (actionMessage === '') {
    ...
    clipsElements.forEach((clip, index) => {
      const { title, streamerId, clipLink, thumbnailUrl } = getClipData(clip);
    ...
    description += '\n';
    embedMessage = new EmbedBuilder().setColor('#d22ef1').setImage(thumbnailUrl).setDescription(description);
  }
